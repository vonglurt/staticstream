...
fn main() -> ExitCode {
    ...
    if matches!(first.as_deref(), Some("-V" | "--version")) {
        println!("sstr-workspace {} (Static Stream format v{})", env!("CARGO_PKG_VERSION"), staticstream::FORMAT_VERSION);
    ...
    if let Some(p) = staticstream_ytq::Paths::from_env() {
