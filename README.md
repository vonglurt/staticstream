...
What phase 0 does:
- **`sstr paths`** shows where ytq's config, queue and log are on this
  machine.
- **The other verbs** say which phase writes them, and point at the
  prototype that already does.
- **No `ytq` binary is built yet.** `~/.local/bin` comes before
  `/usr/local/bin` on Copal's PATH, so an unfinished Rust `ytq` would hide
  the working Python one. It is added when phase 2's test passes.

## Build

```sh
make            # the list
make check      # no external crates, the tests, an offline release build
make run ARGS=paths
make workspace
...
| Crate | What |
|---|---|
| `staticstream` | the format: layout, Reed–Solomon, parity, checkpoints |
| `staticstream-tty` | the armor: a byte stream as 80-column CRC-checked lines for a serial console |
